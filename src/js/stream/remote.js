...
/**
 *    Interface to manage the connection to a Stream server.
 *
...
Remote.flags = {
  ...
  account_root: {
    ...
    DisallowSTM:     0x00080000, // True, to disallow sending STM.
  ...
  state: {
    ...
    LowAuth:         0x00040000,
    HighAuth:        0x00080000,
    LowNoStream:     0x00100000,
    HighNoStream:    0x00200000
  }
};
...
/**
 * Disconnect from the Stream network.
 *
 * @param {Function} callback
 * @api public
 */
...
Remote.prototype._handleMessage = function(message, server) {
  ...
  if (!Remote.isValidMessage(message)) {
    ...
    this.emit('error', new StreamError('remoteUnexpected', 'Unexpected response from remote'));
    return;
  }
  ...
  if (type === 'account_root') {
    ...
    request.request = function() {                        // Intercept default request.
      ...
      //if (self._ledger_hash) {
        ...
        // else if ('stream_state' === request.type)         // YYY Could be cached per ledger.
      //}
...
Remote.prototype.requestLedgerAccept = function(callback) {
  if (!this._stand_alone) {
    this.emit('error', new StreamError('notStandAlone'));
    return;
  }
...
Remote.prototype.requestStreamBalance = function(account, issuer, currency, ledger, callback) {
  if (typeof account === 'object') {
    var options = account;
    callback = issuer;
    ledger   = options.ledger;
  ...
  var request = this.requestLedgerEntry('stream_state'); // YYY Could be cached per ledger.

  request.streamState(account, issuer, currency);
  request.ledgerChoose(ledger);

  function streamState(message) {
  ...
  request.once('success', streamState);
  request.callback(callback, 'stream_state');
...
Remote.prototype.requestStreamPathFind = function(src_account, dst_account, dst_amount, src_currencies, callback) {
  if (typeof src_account === 'object') {
    var options = src_account;
  ...
  var request = new Request(this, 'stream_path_find');
 ...
 * @return {Amount} Final fee in STM for specified number of fee units.
 */
