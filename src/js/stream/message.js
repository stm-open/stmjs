...
Message.MAGIC_BYTES    = 'Stream Signed Message:\n';
 ...
 *  the string 'Stream Signed Message:\n' prepended.
 *
 ...
 *  @param {StreamAddress} [The first key] account Field to specify the signing account. 
 ...
 *  The stream-lib remote is used to check that the public
 ...
 *  @param {StreamAddress} data.account
 *  @param {Base64-encoded String} data.signature
 *  @param {stream-lib Remote} remote
...
Message.verifyHashSignature = function(data, remote, callback) {
  ...
  if (!account || !UInt160.from_json(account).is_valid()) {
    return callback(new Error('Account must be a valid stream address'));
  }
