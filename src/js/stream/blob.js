...
/**
 * Get stream name for a given address
 */

BlobClient.getStreamName = function(url, address, fn) {
  if (!crypt.isValidAddress(address)) {
    return fn (new Error('Invalid stream address'));
  }

  if (!crypt.isValidAddress(address)) return fn (new Error("Invalid stream address"));
  request.get(url + '/v1/user/' + address, function(err, resp){
    if (err) {
      ...
      fn (new Error('No stream name for this address'));
    } else {
      fn(new Error('Unable to determine if stream name exists'));
    }
  });
};
...
BlobClient.updateAvatar = function (url, id, form, fn) {
  var config = {
    method : 'POST',
    url    : url + '/v1/blob/' + id + '/avatar/update',
    formData   : form
  };
  request.post({url: config.url,formData: form},function(err, resp) {
      if (err) {
        fn(err);
      } else if (resp.body && resp.body.result === 'success') {
        fn(null, resp.body);
        ...
        fn(new Error('Unable to update avatar.'));
      }
    });
};
