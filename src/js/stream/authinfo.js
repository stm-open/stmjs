...
var StreamTxt  = require('./streamtxt').StreamTxt;

var AuthInfo = { };

AuthInfo._getStreamTxt = function(domain, callback) {
  StreamTxt.get(domain, callback);
};
...
AuthInfo.get = function(domain, username, callback) {
  ...
  function getStreamTxt(callback) {
    self._getStreamTxt(domain, function(err, txt) {
      if (err) {
        return callback(err);
      }
  ...
  async.waterfall([ getStreamTxt, getUser ], callback);
};
