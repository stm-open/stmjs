// Represent Stream amounts and currencies.
...
function Amount() {
  ...
  this._offset      = 0; // Always 0 for STM.
  this._is_native   = true; // Default to STM. Only valid if value is not NaN.
...
var consts = {
  ...
  // Maximum possible amount for non-STM currencies using the maximum mantissa
 ...
 * a simplification, which should be sane in most cases. For example, a USD/STM
 ...
 *   should be applied. Can be given as JavaScript Date or int for Stream epoch.
...
Amount.prototype.ratio_human = function(denominator, opts) {
  ...
  // Special case: The denominator is a native (STM) amount.
  //
  // In that case, it's going to be expressed as base units (1 STM =
 ...
 * retains STMs human/external interpretation (i.e. 1 STM = 1,000,000 base
 * units).
 *
 * Intended use is to calculate something like: 10 USD * 10 STM/USD = 100 STM
 *
 * @example
...
Amount.prototype.product_human = function(factor, opts) {
  ...
  // Special case: The second factor is a native (STM) amount expressed as base
  // units (1 STM = 10^xns_precision base units).
  //
 ...
 * Examples:
 *
 *   STM 250     => 250000000/STM
 *   25.2 STM    => 25200000/STM
...
Amount.prototype.parse_human = function(j, opts) {
  ...
  if (integer) {
    ...
    // STM have exactly six digits of precision
    if (currency === 'STM') {
 ...
 * @param opts.reference_date {Date|Number} Date based on which demurrage/interest
 *   should be applied. Can be given as JavaScript Date or int for Stream epoch.
 * @param opts.stm_as_drops {Boolean} Whether STM amount should be treated as
 *   drops. When the base currency is STM, the quality is calculated in drops.
 *   For human use however, we want to think of 1000000 drops as 1 STM and
 *   prices as per-STM instead of per-drop.
 */
Amount.prototype.parse_quality = function(quality, counterCurrency, counterIssuer, opts)
{
  ...
  // Correct offset if stm_as_drops option is not set and base currency is STM
  if (!opts.stm_as_drops &&
  ...
  switch (typeof j) {
    ...
    case 'object':
      ...
      if (j instanceof Amount) {
        ...
        this._currency.parse_json(j.currency, true); // Never STM.
...
// Parse a STM value from untrusted input.
// - integer = raw units
...
Amount.prototype.to_human = function(opts) {
  ...
  var int_length = int_part.length || 0;
  if (opts.group_sep) {
  ...
  if (opts.max_length) {
    var fraction_length = opts.max_length - int_length;
    if (fraction_length > 1) {
      var fraction = fraction_part.length ? '.' + fraction_part : '';
      formatted += fraction.slice(0, fraction_length);
    }
    return formatted;
  }
...
Amount.prototype.not_equals_why = function(d, ignore_issuer) {
  ...
  if (!this.is_valid() || !d.is_valid()) {
    ...
    var type = this._is_native ? 'STM' : 'Non-STM';
    ...
    if (!this._value.equals(d._value) || this._offset !== d._offset) {
      ...
      if (!this._currency.equals(d._currency)) {
        result = 'Non-STM currency differs.';
      } else if (!ignore_issuer && !this._issuer.equals(d._issuer)) {
        result = 'Non-STM issuer differs: ' + d._issuer.to_json() + '/' + this._issuer.to_json();
      }
    }
  }
