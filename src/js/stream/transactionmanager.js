...
TransactionManager.prototype._request = function(tx) {
  ...
  if (tx.attempts > 10) {
    return tx.emit('error', new StreamError('tejAttemptsExceeded'));
  }
  ...
  if (tx.attempts > 0 && !remote.local_signing) {
    ...
    return tx.emit('error', new StreamError('tejLocalSigningRequired', message));
  }
...
TransactionManager.prototype.submit = function(tx) {
  ...
  tx.once('abort', function() {
    tx.emit('error', new StreamError('tejAbort', 'Transaction aborted'));
  });
