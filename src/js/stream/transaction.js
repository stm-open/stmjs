...
var StreamError      = require('./streamerror').StreamError;
...
Transaction.flags = {
  ...
  TrustSet: {
    SetAuth:            0x00010000,
    NoStream:           0x00020000,
    SetNoStream:        0x00020000,
    ClearNoStream:      0x00040000,
 ...
 *
 * Each Stream transaction based on its type and makeup costs a certain number
 ...
 *
 * @see https://stream.com/wiki/Transaction_Fee
 *
...
Transaction.prototype.complete = function() {
  if (this.remote) {
    if (!this.remote.trusted && !this.remote.local_signing) {
      this.emit('error', new StreamError('tejServerUntrusted', 'Attempt to give secret to untrusted server'));
      return false;
    }
  }
  ...
  if (!this._secret && !(this._secret = this._accountSecret(this.tx_json.Account))) {
    this.emit('error', new StreamError('tejSecretUnknown', 'Missing secret'));
    return false;
  }
  ...
  if (typeof this.tx_json.SigningPubKey === 'undefined') {
    try {
      ...
      this.emit('error', new StreamError('tejSecretInvalid', 'Invalid secret'));
      return false;
    }
  }
  ...
  if (this.remote && typeof this.tx_json.Fee === 'undefined') {
    if (this.remote.local_fee || !this.remote.trusted) {
      if (!(this.tx_json.Fee = this._computeFee())) {
        this.emit('error', new StreamError('tejUnconnected'));
        return;
      }
    }
  }
  ...
  if (Number(this.tx_json.Fee) > this._maxFee) {
    this.emit('error', new StreamError('tejMaxFeeExceeded', 'Max fee exceeded'));
    return false;
  }
...
Transaction.prototype.setFlags = function(flags) {
  ...
  for (var i=0, l=flag_set.length; i<l; i++) {
    ...
    if (transaction_flags.hasOwnProperty(flag)) {
      ...
      return this.emit('error', new StreamError('tejInvalidFlag'));
    }
  }
 ...
 *  "asfDisallowSTM"
 *    STM should not be sent to this account
 *  "asfDisableMaster"
...
Transaction.prototype.offerCreate = function(src, taker_pays, taker_gets, expiration, cancel_sequence) {
  ...
  if (expiration) {
    this.tx_json.Expiration = utils.time.toStream(expiration);
  }
 ...
 *
 *  The RegularKey must be a valid Stream Address, or a Hash160 of
...
Transaction.prototype.setRegularKey = function(src, regular_key) {
  ...
  if (!UInt160.is_valid(regular_key)) {
    throw new Error('RegularKey must be a valid Stream Address (a Hash160 of the public key)');
  }
...
Transaction.prototype.streamLineSet = function(src, limit, quality_in, quality_out) {
  if (typeof src === 'object') {
    var options = src;
...
Transaction.prototype.submit = function(callback) {
  ...
  function transactionError(error, message) {
    if (!(error instanceof StreamError)) {
      error = new StreamError(error, message);
    }
    self.callback(error);
  };
  ...
  if (!UInt160.is_valid(account)) {
    return this.emit('error', new StreamError('tejInvalidAccount', 'Account is missing or invalid'));
  }
