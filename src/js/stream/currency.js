...
var Currency = extend(function() {
  // Internal form: 0 = STM. 3 letter-code.
...
Currency.HEX_CURRENCY_BAD = '00000000000000000000000053544D0000000000';
...
Currency.from_json = function(j, shouldInterpretStmAsIou) {
  if (j instanceof this) {
    return j.clone();
  } else {
    return (new this()).parse_json(j, shouldInterpretStmAsIou);
  }
};
...
Currency.prototype.parse_json = function(j, shouldInterpretStmAsIou) {
  this._value = NaN;
...
Currency.prototype.parse_bytes = function(byte_array) {
  if (Array.isArray(byte_array) && byte_array.length === 20) {
    ...
    if (isZeroExceptInStandardPositions) {
      ...
      if (/^[A-Z0-9]{3}$/.test(currencyCode) && currencyCode !== 'STM' ) {
...
/**
 *
 * @param referenceDate - number of seconds since the Stream Epoch (0:00 on January 1, 2000 UTC)
